...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var origTemp: UITextView!
    @IBOutlet weak var destTemp: UITextView!
    
    @IBOutlet weak var origForcast: UITextView!
    @IBOutlet weak var destForecast: UITextView!
    ...
    func doCalculatations()
    {
        ...
        wAPI.getWeatherForDate(date: Date(), forLocation: (p1lt, p1ln)) { (weather)
            ...
            if let w = weather {
                DispatchQueue.main.async {
                    self.origTemp.text = w.summary
                    self.origImage = UIImage(named: w.iconName)
                    self.origForcast = "\(w.temperature.roundto(places: 1))"
        ...
        wAPI.getWeatherForDate(date: Date(), forLocation: (p2lt, p2ln)) { (weather)
            ...
            if let w = weather {
                DispatchQueue.main.async {
                    // TODO: Bind the weather object attributes to the view here
                    self.destTemp.text = w.summary
                    self.destImage = UIImage(named: w.iconName)
                    self.destForcast = "\(w.temperature.roundto(places: 1))"
                }
            }
        }
