...
//  GeoCalculator
//
//  Created by Kent Sinclair on 3/29/17.
...
class LocationSearchViewController: FormViewController {
    ...
    override func viewDidLoad() {
        ...
        form = Section("Calculator Inputs") <<< LabelRow () { row in
            ...
            row.add(ruleSet: rules) }.onCellSelection { cell, row in
        ...
        <<< LabelRow () { row in
            ...
            rules.add(rule: RuleClosure(closure: { (loc) -> ValidationError? in
                                            if loc == "Tap to search" {
                                                ...
                                                return ValidationError(msg: "You must select a location") } else {
                                                return nil
                                            } }))
            row.add(ruleSet: rules) }.onCellSelection { cell, row in
            // crank up Google's place picker when row is selected.
            ...
            +++ Section("Calculation Date") <<< DateRow() { row in
            row.title = "Date"
            row.value = Date()
        ...
        let cancelButton : UIBarButtonItem = UIBarButtonItem(title: "Cancel", style: .plain,
                                                             target: self,
                                                             action: #selector(LocationSearchViewController.cancelPressed))
        self.navigationItem.leftBarButtonItem = cancelButton
        let saveButton : UIBarButtonItem = UIBarButtonItem(title: "Save", style: .plain,
                                                           target: self,
    ...
    func savePressed()
    {
        ...
        if errors.count > 0 {
            ...
            self.delegate?.set(calculationData: LocationLookup(origLat: p1Lat, origLng: p1Lng,
                                                               destLat: p2Lat,
